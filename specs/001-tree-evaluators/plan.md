...
**性能目标**: 单条线索 3 万节点规模下（真实分支形态，分支因子约 10）：节点追加**中位数
< 20ms 且 p99 < 150ms**、`children()` 枚举（单父节点约 10 个子节点）p99 < 150ms、
按三维索引过滤树 p99 < 200ms；阈值可由 `CINEFLOW_BENCH_*` 环境变量覆盖（本地可收紧自查）。
注 ①：`children()` 契约全量物化返回，病态宽树（单父节点数万子节点）的下限由行传输与
反序列化决定（实测秒级），不构成目标形态；注 ②：单一 p99 门槛在共享 CI runner 上被
调度/IO 离群停摆主导（实测 50.7ms 擦线误报），故以中位数守真实退化、p99 守灾难性停摆。
