"""声音做梦一轮接入验证（功能 006 / T625，C15）。

agent_id="sound" 演示档 M=8 跑 dreaming 管线：候选生成 → 静态检查 →
沙箱回放（测试档 in_process_replay，同一管线函数）→ reward 排名 →
首轮基线落盘 history_root/sound/dream-sound-1.json。
research 决策 8：dreaming 对 agent_id 已泛化（005），本套件验证零改动接入；
champion 策略与谱系（policies/history/sound/）单源加载（T624 落盘）。
"""
...
SOUND_POLICY_DIR = REPO_ROOT / "policies" / "history" / "sound"


def _champion():
    """champion 单源加载：目录内唯一 {version}.py + 配套 meta.json。"""
    sources = sorted(SOUND_POLICY_DIR.glob("*.py"))
    assert len(sources) == 1, "policies/history/sound/ 应恰有一个 champion 版本文件"
    path = sources[0]
    source = path.read_text(encoding="utf-8")
    spec = importlib.util.spec_from_file_location("sound_champion", path)
...
class TestChampion落盘纪律:
    ...
    def test_meta_谱系首版合法(self):
        ...
        meta = json.loads((SOUND_POLICY_DIR / f"{version}.meta.json").read_text(encoding="utf-8"))
        validate_meta(meta)  # schema 校验（dreaming/lineage 口径）
        assert meta["version"] == version
        assert meta["parent_version"] is None  # 首版谱系根
        assert meta["source"] == "manual"

    def test_网格为合法_SoundGenParams(self):
        _, _, grid = _champion()
        gen_types = [item["gen_type"] for item in grid]
        assert gen_types == ["tts", "tts", "sfx", "music"]  # 2+1+1 单轮配比
        assert all("seed" in item for item in grid)


def _dream_pool(tree_store, make_tree, make_node, grid, n_trees=3):
    """声音做梦池：N 棵不同时间树，子节点 gen_params = champion 网格（逐树得分递增）。"""
    trees = []
    for t in range(n_trees):
        tree = make_tree(
            agent_id="sound",
            project_id="sound-dream",
            config_snapshot={
        ...
        for i, params in enumerate(grid):
            tree_store.append_node(
                make_node(
                    ...
                    cost=CostRecord(llm_calls=1),  # 历史虚拟成本（生成调用不入回放审计口径）
                    status=NodeStatus.EVALUATED,
...
class Test做梦一轮接入:
    def test_演示档_M8_全流程(self, tree_store, make_tree, make_node, dream_config, tmp_path):
        """C15 场景 3：一轮做梦（M=8）→ reward 排名 + 首轮基线落盘，LLM/生成零调用。"""
        ...
        assert dream_round.agent_id == "sound"
    ...
    def test_二轮_seq_递增不覆盖(self, tree_store, make_tree, make_node, dream_config, tmp_path):
        """落盘只增不改：二轮产出 dream-sound-2.json，首轮基线原样保留。"""
        _, champion_source, grid = _champion()
        pool = _dream_pool(tree_store, make_tree, make_node, grid)
        history_root = tmp_path / "dreaming" / "history"
        for seed in ("sound-seed-1", "sound-seed-2"):
            run_dream_round(
                "sound",
                champion_source,
        ...
        first = history_root / "sound" / "dream-sound-1.json"
        second = history_root / "sound" / "dream-sound-2.json"
        assert first.exists() and second.exists()
        assert json.loads(first.read_text(encoding="utf-8"))["round_id"] == "dream-sound-1"

    def test_dreaming_零改动证明(self):
        """dreaming 管线无 sound 特判分支（泛化缺口 = 0 的静态证明）。"""
        import inspect

        import dreaming.pipeline

        source = inspect.getsource(dreaming.pipeline)
        assert '"sound"' not in source and "'sound'" not in source
