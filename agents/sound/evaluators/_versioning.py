"""声音评估器共享工具：实现哈希版本号（research 决策 3/4，004 `_versioning.py` 同款）。

版本号 = 1.0.0+<实现文件内容与分档/阈值/口径参数哈希前12位>——
...
def implementation_version(*parts: str, base: str = "1.0.0") -> str:
    """版本号：base+<实现文件与附加部件（分档/阈值/校准带）哈希前 12 位>。"""
    hasher = blake3.blake3()
    hasher.update(Path(__file__).parent.parent.name.encode())  # 包路径标识（sound）
