"""迁移 0006：edit_render_jobs 可变运营表（剪辑闭环两段式落盘的中间态载体）

- 唯一键 (round_id, edl_hash)：同轮次同 EDL 重复提交直接命中约束（幂等）；
- 实际扣费 ≤ 预估（actual_cost_usd <= estimated_cost_usd）落进 CHECK；
- 运营表不适用 immutable 触发器；节点在评估完成后一次性 INSERT 落树冻结。

修订 ID: 0006_edit_render_jobs
父修订: 0005_sound_gen_jobs
"""
...
revision: str = "0006_edit_render_jobs"
down_revision: str | Sequence[str] | None = "0005_sound_gen_jobs"
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("edl_json", sa.Text(), nullable=False),
        sa.Column("edl_hash", sa.Text(), nullable=False),
        sa.Column(
            "status",
            sa.Text(),
            sa.CheckConstraint(
        ...
        sa.UniqueConstraint("round_id", "edl_hash", name="uq_edit_round_edl"),
    )
    # 应用账号对运营表有完整读写权限（与 immutable 树表刻意区分，同 0005）
    op.execute("GRANT SELECT, INSERT, UPDATE, DELETE ON edit_render_jobs TO cineflow_app")
    # 显式 schema USAGE：schema 重建后默认授权会消失（一期 CI 教训，同 0004）
    op.execute("GRANT USAGE ON SCHEMA public TO cineflow_app")


def downgrade() -> None:
    op.drop_table("edit_render_jobs")
